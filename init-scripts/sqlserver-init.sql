...
-- Create test database and tables with VARBINARY for binary data
...
-- Products Table (with images as VARBINARY)
...
INSERT INTO migration_test.products (name, description, price, image, thumbnail) VALUES
    ('Laptop Pro', 'High-performance laptop', 1299.99,
        CONVERT(VARBINARY(MAX), 'PNG_IMAGE_DATA_1'),
        CONVERT(VARBINARY(MAX), 'PNG_THUMB_1')),
    ('Wireless Mouse', 'Ergonomic wireless mouse', 29.99,
        CONVERT(VARBINARY(MAX), 'PNG_IMAGE_DATA_2'),
        CONVERT(VARBINARY(MAX), 'PNG_THUMB_2')),
    ('USB-C Hub', 'Multi-port USB-C hub', 49.99,
        CONVERT(VARBINARY(MAX), 'PNG_IMAGE_DATA_3'),
        CONVERT(VARBINARY(MAX), 'PNG_THUMB_3')),
    ('Mechanical Keyboard', 'RGB mechanical keyboard', 149.99,
        CONVERT(VARBINARY(MAX), 'PNG_IMAGE_DATA_4'),
        CONVERT(VARBINARY(MAX), 'PNG_THUMB_4')),
    ('Monitor 4K', '27-inch 4K monitor', 399.99,
        CONVERT(VARBINARY(MAX), 'PNG_IMAGE_DATA_5'),
        CONVERT(VARBINARY(MAX), 'PNG_THUMB_5'));
