...
public class ScriptGenerationService : DatabaseServiceBase
{
        ...
        IProgress<ScriptGenerationProgress>? progress, ScriptGenerationProgress state, CancellationToken ct)
    {
        ...
        //  - SQL Server target: wrap the INSERTs in SET IDENTITY_INSERT ON/OFF — exactly as SSMS "Generate Scripts"
        //    does — otherwise they fail with "An explicit value for the identity column ... can only be specified
        //    when IDENTITY_INSERT is ON". OFF also reseeds the identity, so nothing else is needed there.
        //  - PostgreSQL/Oracle target: the column is GENERATED BY DEFAULT AS IDENTITY, so explicit inserts are
        //    allowed but do NOT advance the backing sequence; we resync it after the load (see below), otherwise
        //    the next server-generated INSERT collides on the primary key.
        var sourceColumns = await _schemaService.GetTableColumnsAsync(
            connection, sourceDbType, table.Schema, table.Name);
        var identityColumns = sourceColumns.Where(c => c.IsIdentity).ToList();
        bool wrapIdentityInsert = identityColumns.Count > 0 && dialect == DatabaseType.SqlServer;
        ...
        async Task FlushBatchAsync()
        {
            ...
            // SSMS style: SET IDENTITY_INSERT ON is emitted once, lazily, right before the first INSERT that
            // actually carries rows (so empty tables get no wrapper) and is paired with OFF in the finally below.
            if (wrapIdentityInsert && !identityInsertOn)
            {
                await output.WriteLineAsync($"SET IDENTITY_INSERT {targetTable} ON");
                await output.WriteLineAsync("GO");
        ...
        {
            // Always close the IDENTITY_INSERT scope we opened, even if reading/formatting threw mid-table, so the
            // generated script stays runnable (SQL Server allows IDENTITY_INSERT ON for only one table at a time).
            if (identityInsertOn)
            {
                await output.WriteLineAsync($"SET IDENTITY_INSERT {targetTable} OFF");
                await output.WriteLineAsync("GO");
            }
        }

        // PostgreSQL/Oracle: realign each identity column's sequence to the rows just loaded (pg_dump style), so
        // the first later server-generated value continues after the migrated data instead of colliding on the PK.
        if (rowsForTable > 0 && (dialect == DatabaseType.PostgreSQL || dialect == DatabaseType.Oracle))
        {
            foreach (var idCol in identityColumns)
            {
                string? resync = BuildIdentitySequenceResyncStatement(dialect, table.Schema, table.Name, idCol.Name);
                if (!string.IsNullOrEmpty(resync))
                    await output.WriteLineAsync(resync);
            }
        }
